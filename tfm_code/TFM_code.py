...
SCENARIOS = {
    'simple':  {'n_factors': 6,  'items_per_factor': 5},   # 30 ítems
    'complex': {'n_factors': 12, 'items_per_factor': 5},   # 60 ítems
}

# Colores para figuras
COLORS = {
    'baseline':    '#a8dadc',   # verde agua claro — claramente distinto de MICE
    'mice':        '#1d3557',   # azul marino oscuro — contrasta bien con baseline
    'autoencoder': '#e63946',   # rojo coral — bien diferenciado de los azules
    'original':    '#2d6a4f',   # verde oscuro — para UMAP datos originales
    'missing':     '#f4a261',   # naranja — para UMAP datos con missing
}
...
def simulate_psychometric_data(n_participants, n_factors, items_per_factor, seed=None):
    """
    Simula datos psicométricos con estructura factorial.
    - Cargas factoriales: 0.50–0.80
    - Correlaciones entre factores: 0.20–0.35
    - Escala Likert 1–5 (datos continuos discretizados)
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    factor_corr = np.full((n_factors, n_factors), 0.275)  # media de 0.20-0.35
    ...
    # Sin cargas cruzadas: cada ítem carga solo en su factor
    ...
    return pd.DataFrame(data_likert, columns=columns)
...
def introduce_missing(data, mechanism, missing_rate, seed=None):
    ...
    if mechanism == 'MCAR':
        ...
        # La probabilidad de missing depende de un subconjunto de variables observadas
        # (primer factor completo) mediante funcion logistica pronunciada (k=2.5).
        # Genera patrones MAR claramente diferenciados del MCAR, siguiendo
        # Van Buuren (2018) y Madley-Dowd et al. (2019).
        # Justificacion sustantiva: en evaluacion de personal es plausible que la
        # probabilidad de omitir items de un factor dependa del nivel observado en otro.
        n_anchor = max(1, p // 6)  # primer factor completo (aprox. items_per_factor items)
...
def run_reclassification_analysis(n_factors, items_per_factor, mechanisms_target,
    ...
    for mechanism in mechanisms_target:
        ...
        for rep in range(n_reps):
            ...
            # Datos originales
            data_orig = simulate_psychometric_data(
...
def plot_rmse_interaction(df_results, save_path='figura_interaccion_rmse.png'):
    """
...
def plot_loss_curves(all_loss_curves, save_path='figura_curva_loss_autoencoder.png'):
    """
...
def plot_umap_comparison(df_results, save_path='figura_umap_comparacion.png'):
    """
...
def plot_nrmse_by_method(df_results, save_path='figura_nrmse_metodos.png'):
    """
...
def plot_corr_preservation(df_results, save_path='figura_preservacion_correlaciones.png'):
    """
...
def plot_me_bias(df_results, save_path='figura_sesgo_medio_me.png'):
    """
...
if __name__ == '__main__':
    ...
    print("  🖼️  figura_interaccion_rmse.png")
    print("  🖼️  figura_curva_loss_autoencoder.png")
    print("  🖼️  figura_nrmse_metodos.png")
    print("  🖼️  figura_preservacion_correlaciones.png")
    print("  🖼️  figura_sesgo_medio_me.png")
    print("  🖼️  figura_umap_comparacion.png")
    print("=" * 60)